...
Roblox.Services.Games = class extends Extension.BackgroundService {
  constructor() {
    ...
    let securityParameters = $.param({
      verification: Extension.Singleton.id,
      _: +new Date(),
    });
    ...
    this.refererOverrideValue =
      'https://www.roblox.com/users/48103520/profile?roblox=plus';
    this.launchMessenger = new Extension.Messaging(
      this.extension,
      `Extension.BackgroundService.${this.serviceId}.ProtocolLaunchMessenger`,
      this.tryLaunchProtocolUrl.bind(this)
    );
  ...
  getAuthTicket() {
    return QueuedPromise(
      `${this.serviceId}.getAuthTicket`,
      (resolve, reject) => {
        $.post(this.authTicketUrl)
          .done((r, status, xhr) => {
  ...
  launch(launchArguments) {
    return new Promise((resolve, reject) => {
      ...
      if (launchArguments.hasOwnProperty('followUserId')) {
        if (
          typeof launchArguments.followUserId != 'number' ||
          launchArguments.followUserId <= 0
        ) {
          reject([
            {
              code: 0,
        ...
        if (
          typeof launchArguments.placeId != 'number' ||
          launchArguments.placeId <= 0
        ) {
          reject([
            {
              code: 0,
        ...
        if (
          typeof launchArguments.serverId != 'string' &&
          launchArguments.hasOwnProperty('serverId')
        ) {
          reject([
            {
              code: 0,
        ...
        if (launchArguments.serverId) {
          this.trackJoinedServer(launchArguments.serverId)
            .then(() => {
  ...
  tryLaunchProtocolUrl(message) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        Extension.ExecutionContextTypes.tab
      ) {
...
switch (Extension.Singleton.executionContextType) {
  ...
  case Extension.ExecutionContextTypes.tab:
    $(
      '<a href="javascript:Roblox=window.Roblox||{};(Roblox.VideoPreRollDFP||Roblox.VideoPreRoll||{}).showVideoPreRoll=false;">'
    )[0].click();

    setInterval(function () {
      var gameServerSrc = $('#gamelaunch').attr('src');
      if (gameServerSrc) {
        var gameServerIdMatch = decodeURIComponent(gameServerSrc).match(
          /accessCode=([\w\-]+)/i
        ) || [''];
