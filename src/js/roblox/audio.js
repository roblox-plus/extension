...
Roblox.Services.Audio = class extends Extension.BackgroundService {
  constructor() {
    ...
    this.register([this.getSoundUrl]);
  }
  ...
  createPlayButton(defaultVolume) {
    ...
    button.setAudioId = function (assetId) {
      return button
        .attr('data-' + Extension.Singleton.id + '-robloxsound', assetId)
        .addClass('icon-play')
        .removeClass('icon-pause icon-audio icon-brokenpage');
    };
...
if (
  ...
  $(function () {
    ...
    $('body')
      .on('click', '.icon-play[' + attrName + ']', function () {
            ...
            .then(function (player) {
              ...
              player
                .play(function () {
                  $('span' + searchAttr)
                    .addClass('icon-pause')
                    .removeClass('icon-play icon-audio');
                })
                .stop(function () {
                  $('span' + searchAttr)
                    .addClass('icon-play')
                    .removeClass('icon-pause icon-audio');
                })
                .error(function () {
                  $('span' + searchAttr)
                    .addClass('icon-brokenpage')
                    .removeClass('icon-play icon-audio icon-pause');
                });
              player.play(volume);
            })
            .catch(function (e) {
