...
brickColor = {
	...
	new: function (c, g, b) { var cs = tostring(c); if (typeof (c) == "object" && c.name) { return this.create(c.name); } else if (g != _ && b != _) { return this.create(this.numList[this.fromRGB(c, g, b).number]); } else if (cs.startsWith("rgb")) { var s = cs.split(","); return brickColor.new(round(s[0]), s[1], s[2]); } else if (toNumber(c) > 0) { return this.create(this.numList[round(c)] || "Medium stone grey"); } return this.create(c); },
