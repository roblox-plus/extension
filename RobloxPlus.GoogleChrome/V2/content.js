...
$(".MediaPlayerIcon[data-mediathumb-url]").each(function(){$(this).replaceWith(soundService.robloxSound.button($(this).attr("data-mediathumb-url"),$(this).prop("tagName")));});
	...
	match: /^\/users\/(\d*)\/?friends/i,
	fill: function(id){
		id = Number(id[1]) || users.userId;
	...
	match: /^\/users\/(\d+)\/inventory/i,
	fill: function(userId){
		if(!(userId = Number(userId[1]))){return;}
	...
	match: [/^\/my\/groups\.aspx/i,/^\/groups\/group\.aspx/i],
	fill: function(id){
		if(!(id=Number(url.param("gid")||url.param("id",$("#ctl00_cphRoblox_AbuseReportButton_ReportAbuseTextHyperLink").attr("href"))))){return;}
