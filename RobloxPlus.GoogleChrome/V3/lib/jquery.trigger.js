/*
	...
	broadcast(event[, ...]); // When a namespace is given this will fire the event in all content, and background scripts with the given arguments
	broadcast.key = namespace; // The key it broadcasts with. If you change this you will have to change it for all listeners, and in the background page.
*/
$.addTrigger = function(obj, namespace){
	var events = {};
	var callBacks = [];
	var beforeTrigger = {};
	...
	var broadcastKeyPrefix = "$.jquery.trigger.broadcast.";
	...
	obj.broadcast = function(){
		...
		if(ext.isBackground){
			...
			request.tabs.forEach(function(id){
				request.send({
					request: broadcastKeyPrefix + obj.broadcast.key,
					data: data
				}, undefined, id);
			});
		}else{
	...
	obj.broadcast.key = namespace;

	obj.broadcast.requester = request.sent(function(data, callBack, sender){
		if(data.request == broadcastKeyPrefix + obj.broadcast.key){
			if(ext.isBackground){
				obj.broadcast.apply(obj, data.data);
			}else{
				obj.trigger.apply(obj, data.data);
			}
		}
	}).connect();
